package com.rj.keys.infraestructure.persistence.adapter;
...
import com.rj.keys.infraestructure.persistence.repository.KeysRedisRepository;
import com.rj.keys.utils.KeyGeneratorUtils;
...
public class KeysAdapters implements Keys_RepositoryPort {
    ...
    public public_keys_Entity SetKey() {
        List<public_keys_Entity> existsKeys = new ArrayList<>();
        try {
            ...
            if (existsKeys.isEmpty()) {
                ...
                public_keys_Entity oldKey = existsKeys.stream().filter(k -> k.getId() == 2).findFirst().orElse(existsKeys.get(1));
                repository.delete(oldKey);

                public_keys_Entity currentKey = existsKeys.stream().filter(k -> k.getId() == 1).findFirst().orElse(existsKeys.get(0));
    ...
    private public_keys_Entity saveNewKey(int id, String pub, String priv, boolean state, LocalDateTime now) {
        public_keys_Entity newKey = new public_keys_Entity();
        newKey.setId(id);
    ...
    private void rotateKey(public_keys_Entity key, int newId, boolean newState, LocalDateTime now) {
